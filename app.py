...
# Application
app = Starlette(
                debug=False, # set to false when not working
...
#        Route('/egen', utvalg_egen), # TODO remove
        Route('/om', om_tjenesten),
        Route('/f/{fylke}', kom_fylk), # TODO remove
        Route('/k/{kom}', kom_fylk), # TODO remove
        Mount('/css', StaticFiles(directory="static"), name="css")
        ]
)
...
#        Route('/{s:path}', fritekst, methods=["POST"])),
        ...
        Route('/robots.txt', robots_nodebug_txt),
        Route('/debug_info', debug_output_state),
